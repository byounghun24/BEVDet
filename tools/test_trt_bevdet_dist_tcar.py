...
# -*- coding: utf-8 -*-
"""
Multi-GPU TensorRT evaluation script for BEVDet (TCAR only).
...
Example usage:
torchrun --nproc_per_node=4 tools/test_trt_bevdet_dist_tcar.py \
  configs/bevdet/bevdet-r50-tcar.py \
  /home/byounghun/workspace/BEVDet/tensorrt/bevdet_tcar_dynamic_int8_fuse.engine \
  --metric bbox \
  --out-dir outputs/trt_eval_tcar
"""
...
def parse_args():
    ...
    parser.add_argument('--out-dir', default='outputs/trt_eval_tcar')
...
def main():
    ...
    if dataset_type is not None and dataset_type != 'TcarDataset':
        raise RuntimeError(
            f'TCAR TRT test expects dataset_type="TcarDataset", got "{dataset_type}". '
            'Use the nuScenes TRT test script for non-TCAR configs.')
    if isinstance(cfg.data.get('test', None), dict):
        test_type = cfg.data.test.get('type', None)
        if test_type is not None and test_type != 'TcarDataset':
            raise RuntimeError(
                f'TCAR TRT test expects test dataset type "TcarDataset", got "{test_type}".')
