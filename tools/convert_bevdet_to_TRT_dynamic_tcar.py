...
'''
TCAR-specific dynamic TRT export (does NOT touch nuScenes scripts).

Example:
python tools/convert_bevdet_to_TRT_dynamic_tcar.py \
  configs/bevdet/bevdet-r50-tcar.py \
  /home/byounghun/workspace/BEVDet/ckpts/bevdet-r50-tcar.pth \
...
def parse_args():
    ...
    parser.add_argument(
        '--prefix', default='bevdet_tcar', help='prefix of the save file name')
    parser.add_argument(
...
def main():
    ...
    if dataset_type is not None and dataset_type != 'TcarDataset':
        raise RuntimeError(
            f'TCAR export expects dataset_type="TcarDataset", got "{dataset_type}". '
            'Use the nuScenes export script for non-TCAR configs.')
    if isinstance(cfg.data.get('train', None), dict):
        train_type = cfg.data.train.get('type', None)
        if train_type is not None and train_type != 'TcarDataset':
            raise RuntimeError(
                f'TCAR export expects train dataset type "TcarDataset", got "{train_type}".')
