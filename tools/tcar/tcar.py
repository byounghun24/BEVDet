...
class TestCar(NuScenes):
    ...
    def __init__(self,
        ...
        self.table_names = ['category', 'instance', 'sensor', 'calibrated_sensor',
                            'ego_pose', 'scene', 'sample', 'sample_data', 'sample_annotation']
    ...
    def get_box(self, sample_annotation_token: str) -> Box:
        ...
        # Some datasets (e.g., TCAR) may not provide velocity in annotations.
        velocity = record.get('velocity', [0.0, 0.0, 0.0])
        category_name = record.get('category_name')
        if category_name is None:
            inst = self.get('instance', record['instance_token'])
            category_name = self.get('category', inst['category_token'])['name']
        return Box(record['translation'], record['size'], Quaternion(record['rotation']),
                   name=category_name, token=record['token'], velocity=velocity)
            ...
            for curr_ann_rec in curr_ann_recs:
                ...
                if curr_ann_rec['instance_token'] in prev_inst_map:
                    ...
                    box = Box(center, curr_ann_rec['size'], rotation, name=curr_ann_rec['category_name'],
                              token=curr_ann_rec['token'],  velocity=curr_ann_rec['velocity'])
                else:
...
#         """
#         Loads database and creates reverse indexes and shortcuts.
#         Default behavior matches the official nuScenes-devkit.
#         :param version: Version to load (e.g. "v1.0", ...).
#         :param dataroot: Path to the tables and data.
#         :param verbose: Whether to print status messages during load.
#         :param map_resolution: Resolution of maps (meters).
#         """
#         super().__init__(version=version, dataroot=dataroot, verbose=verbose, map_resolution=map_resolution)
#         # Keep devkit defaults; do not eagerly fill missing TCAR fields.

#     def _ensure_sample_data_fields(self):
#         sensor_idx = {s['token']: s for s in self.sensor}
#         calib_idx = {c['token']: c for c in self.calibrated_sensor}
#         sample_idx = {s['token']: s for s in self.sample}

#         for sd in self.sample_data:
#             cs = calib_idx.get(sd.get('calibrated_sensor_token', ''))
#             if cs is not None:
#                 srec = sensor_idx.get(cs.get('sensor_token', ''))
#                 if srec is not None:
#                     sd.setdefault('channel', srec.get('channel', srec.get('modality', '')))
#                     sd.setdefault('sensor_modality', srec.get('modality', ''))

#             if sd.get('sensor_modality') == 'camera':
#                 if 'width' not in sd or 'height' not in sd:
#                     fpath = osp.join(self.dataroot, sd['filename'])
...
#         # Build prev/next chain per (scene, channel) if missing.
#         groups = defaultdict(list)
#         for sd in self.sample_data:
#             scene_token = sample_idx.get(sd['sample_token'], {}).get('scene_token', '')
#             key = (scene_token, sd.get('channel', ''))
#             groups[key].append(sd)
#         for items in groups.values():
#             items.sort(key=lambda r: r.get('timestamp', 0))
#             for i, sd in enumerate(items):
#                 sd['prev'] = items[i - 1]['token'] if i > 0 else ''
#                 sd['next'] = items[i + 1]['token'] if i + 1 < len(items) else ''
...
#                         no_transform_boxes: bool = False) -> \
#             Tuple[str, List[Box], np.array]:
#         """
#         Returns the data path as well as all annotations related to that sample_data.
#         Default matches nuScenes-devkit: boxes are returned in the sensor frame.
#         Use boxes_in_global/no_transform_boxes to preserve legacy behavior.
#         :param sample_data_token: Sample_data token.
#         :param box_vis_level: If sample_data is an image, this sets required visibility for boxes.
#         :param selected_anntokens: If provided only return the selected annotation.
#         :param use_flat_vehicle_coordinates: Instead of the current sensor's coordinate frame, use ego frame which is
#                                              aligned to z-plane in the world.
#         :param boxes_in_global: If True, return boxes in global frame (legacy behavior).
#         :param no_transform_boxes: Alias for boxes_in_global (legacy behavior).
#         :return: (data_path, boxes, camera_intrinsic <np.array: 3, 3>)
#         """
#         if boxes_in_global or no_transform_boxes:
#             data_path = self.get_sample_data_path(sample_data_token)
#             sd_record = self.get('sample_data', sample_data_token)
#             cs_record = self.get('calibrated_sensor', sd_record['calibrated_sensor_token'])
#             sensor_record = self.get('sensor', cs_record['sensor_token'])
#             cam_intrinsic = np.array(cs_record['camera_intrinsic']) if sensor_record['modality'] == 'camera' else None
#             boxes = list(map(self.get_box, selected_anntokens)) if selected_anntokens is not None else self.get_boxes(sample_data_token)
#             return data_path, boxes, cam_intrinsic
#         # Default devkit behavior with lazy width/height handling (no JSON mutation).
#         sd_record = self.get('sample_data', sample_data_token)
#         cs_record = self.get('calibrated_sensor', sd_record['calibrated_sensor_token'])
#         sensor_record = self.get('sensor', cs_record['sensor_token'])
#         pose_record = self.get('ego_pose', sd_record['ego_pose_token'])
...
#         if sensor_record['modality'] == 'camera':
#             cam_intrinsic = np.array(cs_record['camera_intrinsic'])
#             if 'width' in sd_record and 'height' in sd_record:
#                 imsize = (sd_record['width'], sd_record['height'])
#             else:
#                 try:
#                     with Image.open(data_path) as _im:
#                         imsize = _im.size  # (w, h)
#                 except Exception:
#                     imsize = None
#         else:
...
#             if sensor_record['modality'] == 'camera' and imsize is not None:
...
#         """
#         record = self.get('sample_annotation', sample_annotation_token)
#         velocity = record.get('velocity', None)
#         if velocity is None:
#             try:
...
#                     velocity = curr_ann_rec.get('velocity', None)
#                     if velocity is None:
#                         try:
