...
work_dir = f'./outputs_bh/bevdet-r50-4d-depth-cbgs-tcar-depthkd/run_{_run_id}'
...
model = dict(
    ...
    depth_kd_cfg=dict(
        ...
        use_fg_mask=True,
        # Keep KD map losses on dense teacher resolution.
        dense_spatial=True,
        dense_base_loss=True,
        dense_interp='bilinear',
        # Option 1) Depth gradient distillation.
        # L_grad = |∇D_student - ∇D_teacher|
        grad_loss_weight=0.0,
        # Option 2) Relative depth(order) distillation.
        # Useful when teacher metric scale is noisy/inconsistent.
        # Set loss_weight=0 and relative_loss_weight>0 for pure ordinal KD.
        relative_loss_weight=kd_relative_loss_weight,
        relative_temperature=1.0,
        relative_min_diff=0.0,
        relative_use_log_depth=False,
        # Option 3) Ray KD (camera-ray transmittance KD).
        # Distills cumulative depth distribution along depth bins.
        ray_loss_weight=0.0,
        ray_loss_type='l1',
    ),
)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                project='bevdet',
                name='bevdet-r50-4d-depth-cbgs-tcar-depthkd'))
    ])
